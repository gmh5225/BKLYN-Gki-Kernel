...
static inline struct memsize_rgn_struct *memsize_get_new_rgn(void)
{
...
void memblock_memsize_detect_hole(void)
{
	...
	for_each_memblock_type(idx, (&memblock.memory), rgn) {
		if (idx == 0)
			prev_end = round_down(rgn->base, SZ_1G);
		else
			prev_end = end;
		base = rgn->base;
		...
		if (hole_s < SZ_1G) {
			...
			hole_s1 = round_up(prev_end, SZ_1G) - prev_end;
			if (hole_s1)
